...
class http(AbstractDeviceMgr):
  ...
  def validate_inputs_form_payload(self, mapping_dict, update=False):
    ...
    http_object = device.http.http()
    try:
      if (update == False) or (update == True and str(mapping_dict.get('no_ip_http_server', None)) != ''):
        http_object.no_ip_http_server = mapping_dict.get('no_ip_http_server', None)
      else:
        http_object.no_ip_http_server._empty_tag = True
    except TypeError:
      pass
    try:
      if (update == False) or (update == True and str(mapping_dict.get('no_ip_http_secure_server', None)) != ''):
        http_object.no_ip_http_secure_server = mapping_dict.get('no_ip_http_secure_server', None)
      else:
        http_object.no_ip_http_secure_server._empty_tag = True
    except TypeError:
      pass
