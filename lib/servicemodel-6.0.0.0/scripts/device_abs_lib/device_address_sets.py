...
class address_sets(object):
  #XPATH devices/device/address-sets/address-set
  class address_set(AbstractDeviceMgr):
    ...
    def validate_inputs_form_payload(self, mapping_dict, update=False):
      ...
      for name_iterator in name:
        from servicemodel.controller.devices.device import address_sets
        ...
        try:
          if (update == False) or (update == True and str(mapping_dict.get('virtual_device_name', None)) != ''):
            address_set_object.virtual_device_name = mapping_dict.get('virtual_device_name', None)
          else:
            address_set_object.virtual_device_name._empty_tag = True
        except TypeError:
          pass
    ...
    #XPATH devices/device/address-sets/address-set/address
    class address(AbstractDeviceMgr):
      key_hints = [['name']]
      def getRcpathPayload(self, sdata, dev, address_set_name, mapping_dict):
        dev = get_valid_devices(dev)
        if len(dev) == 0:
          return
        #Input Key Validations
        self.validate_parent_keys( address_set_name)
        ...
        for address_set_name in address_set_name_list:
          ##prepare rcpath
          rcpath = "firewall:address-sets/address-set=%s"%(util.make_interfacename(address_set_name))
          rcpath_list.append(rcpath)
        mapping_dict = kwargs.get('mapping_dict')
      ...
      def create(self, sdata, dev, address_set_name, mapping_dict, addref=True, autocommit=True):
        dev = get_valid_devices(dev)
        if len(dev) == 0:
          return
        #Input Key Validations
      ...
      def update(self, sdata, dev, address_set_name, mapping_dict, addref=True, autocommit=True):
        dev = get_valid_devices(dev)
        if len(dev) == 0:
          return
        #Input Key Validations
      ...
      def delete(self, sdata, dev, address_set_name, mapping_dict, fail_silently=False, remove_reference=False):
        dev = get_valid_devices(dev)
        if len(dev) == 0:
          return
        #Input Key Validations
      ...
      def validate_parent_keys(self, address_set_name):
      ...
      def validate_inputs_form_payload(self, mapping_dict, update=False):
        ...
        address_object_list = []
        for address_name_iterator in address_name:
          from servicemodel.controller.devices.device.address_sets import address_set
    ...
    #XPATH devices/device/address-sets/address-set/sub-address-set
