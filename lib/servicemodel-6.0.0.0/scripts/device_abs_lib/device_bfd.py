...
class bfd(AbstractDeviceMgr):
  ...
  def validate_inputs_form_payload(self, mapping_dict, update=False):
    ...
    bfd_object = device.bfd.bfd()
    try:
      if (update == False) or (update == True and str(mapping_dict.get('interval', None)) != ''):
        bfd_object.interval = mapping_dict.get('interval', None)
      else:
        bfd_object.interval._empty_tag = True
    except TypeError:
      pass
    try:
      if (update == False) or (update == True and str(mapping_dict.get('min_rx', None)) != ''):
        bfd_object.min_rx = mapping_dict.get('min_rx', None)
      else:
        bfd_object.min_rx._empty_tag = True
    except TypeError:
      pass
    try:
      if (update == False) or (update == True and str(mapping_dict.get('multiplier', None)) != ''):
        bfd_object.multiplier = mapping_dict.get('multiplier', None)
      else:
        bfd_object.multiplier._empty_tag = True
    except TypeError:
      pass
