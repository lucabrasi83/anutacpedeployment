...
"""
def grouping_create_wanop_ic_local_props_type2_wanop_endpoint(smodelctx, sdata, dev, **kwargs):
    inputdict = kwargs["inputdict"]
    ...
    log("sobj")
    log(sobj)
    type2_name = rcpath.split("/")[-4].split("=")[1]
    for each_type2_site in util.convert_to_list(sobj.managed_cpe_services.customer.wanop_services.type2_site.type2_site):
        type2_site_name = each_type2_site.site_name
        if type2_site_name == type2_name:
            site_name = each_type2_site.dual_cpe_sites
            rp = each_type2_site.resource_pool
            if "aux" in inputdict["interface_name"] and util.isEmpty(inputdict["profile_name"]):
                ...
                if rcpath1.split("/")[-3] == "wanop-secondary":
                    obj = getLocalObject(sdata, "type2-site")
                    if util.isNotEmpty(obj):
                        for wanop_endpoint in util.convert_to_list(obj.type2_site.type2_site.wanop_primary.wanop_endpoints.wanop_endpoint):
                ...
                cidr_name = cust_name + '_' + type2_name + '_' +aux_cidr
...
Fetch the parent object using getParentObject() API 
"""
def grouping_update_wanop_ic_local_props_type2_wanop_endpoint(smodelctx, sdata, **kwargs):
    pass
...
Fetch the parent object using getParentObject() API 
"""
def grouping_delete_wanop_ic_local_props_type2_wanop_endpoint(smodelctx, sdata, **kwargs):
    pass
