...
def grouping_create_wanop_ic_local_props_type1_wanop_endpoint(smodelctx, sdata, dev, **kwargs):
    inputdict = kwargs["inputdict"]
    ...
    for each_type1_site in util.convert_to_list(sobj.managed_cpe_services.customer.wanop_services.type1_site.type1_site):
        type1_site_name = each_type1_site.site_name
        if type1_site_name == type1_name:
            site_name = each_type1_site.single_cpe_sites
            rp = each_type1_site.resource_pool
            if "aux" in inputdict["interface_name"] and util.isEmpty(inputdict["profile_name"]):
                aux_cidr = inputdict["cidr"]
                cidr_name = cust_name + '_' + type1_site_name + '_' +aux_cidr
                ...
                if util.isNotEmpty(inputdict["interface_ip"]):
                    ...
                    cidr_name = cust_name + '_' + site_name + '_' + aux_cidr
                    ip_addr = IpamPoolID(sdata.getTaskId(), sdata, cidr_name)
                    ip_addr_obj = ip_addr.ipam_pool_obj
...
Fetch the parent object using getParentObject() API 
"""
def grouping_update_wanop_ic_local_props_type1_wanop_endpoint(smodelctx, sdata, **kwargs):
    pass
...
Fetch the parent object using getParentObject() API 
"""
def grouping_delete_wanop_ic_local_props_type1_wanop_endpoint(smodelctx, sdata, **kwargs):
    pass
