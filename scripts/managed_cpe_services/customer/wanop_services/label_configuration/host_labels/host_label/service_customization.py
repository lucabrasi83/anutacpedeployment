...
class ServiceDataCustomization:
    ...
    def process_service_device_bindings(smodelctx, sdata, dev, **kwargs):
      ...
      if modify:
        ...
        cpedeployment.cpedeployment_grouping_lib.label_configuration_def_customization.grouping_create_label_configuration_def_host_labels_host_label(smodelctx, sdata, dev, **kwargs)

        from servicemodel.device_abs_lib import device_label_configuration
        if inputdict['host_label_name'] is not None:
          device_label_configuration.label_configuration.host_label().create(sdata, dev, fill_map_devices_device_label_configuration_host_label(inputdict), addref=True)
    ...
    def process_service_update_data(smodelctx, sdata, **kwargs):
      ...
      if modify:
        ...
        if up_map_devices_device_label_configuration_host_label[1] == 'key-unchanged':
          device_label_configuration.label_configuration.host_label().update(sdata, dev, fill_map_devices_device_label_configuration_host_label(inputdict))
        import cpedeployment.cpedeployment_grouping_lib.label_configuration_def_customization
        cpedeployment.cpedeployment_grouping_lib.label_configuration_def_customization.grouping_update_label_configuration_def_host_labels_host_label(smodelctx, sdata, **kwargs)
    ...
    def process_service_delete_data(smodelctx, sdata, **kwargs):
      ...
      if modify:
        ...
        if dev is None or (isinstance(dev, list) and len(dev) == 0):
          return
        import cpedeployment.cpedeployment_grouping_lib.label_configuration_def_customization
        cpedeployment.cpedeployment_grouping_lib.label_configuration_def_customization.grouping_delete_label_configuration_def_host_labels_host_label(smodelctx, sdata, **kwargs)

def fill_map_devices_device_label_configuration_host_label(inputdict, delete=False):
