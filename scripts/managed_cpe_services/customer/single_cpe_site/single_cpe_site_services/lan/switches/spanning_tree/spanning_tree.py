...
class SpanningTree(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        config = getCurrentObjectConfig(id, sdata, 'spanning_tree', False)
        ...
        if inputdict.get('sequence_number') is None:
          inputdict['sequence_number'] = '131'
        inputdict['options'] = config.get_field_value('options')
        inputdict['mode'] = config.get_field_value('mode')
        inputdict['extend_system_id'] = config.get_field_value('extend_system_id')
        inputdict['vlan_range'] = config.get_field_value('vlan_range')
        ...
        inputkeydict['managed_cpe_services_customer_single_cpe_site_single_cpe_site_services_lan_switches_device'] = sdata.getRcPath().split('/')[-2].split('=')[1]
    ...
    def update(self, id, sdata):
        #Fetch Local Config Object
        config = getCurrentObjectConfig(id, sdata, 'spanning_tree', False)
        pconfig = getPreviousObjectConfig(id, sdata, 'spanning_tree')
        opaque_args = self.opaque_args
        ...
        if pinputdict.get('sequence_number') is None:
          pinputdict['sequence_number'] = '131'
        pinputdict['options'] = pconfig.get_field_value('options')
        pinputdict['mode'] = pconfig.get_field_value('mode')
        pinputdict['extend_system_id'] = pconfig.get_field_value('extend_system_id')
        pinputdict['vlan_range'] = pconfig.get_field_value('vlan_range')
    ...
    @staticmethod
    def getInstance():
        if(SpanningTree._instance == None):
            SpanningTree._instance = SpanningTree()
        return SpanningTree._instance
