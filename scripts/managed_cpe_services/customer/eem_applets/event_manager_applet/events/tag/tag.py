...
class Tag(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        config = getCurrentObjectConfig(id, sdata, 'tag', False)
        ...
        inputdict['tag_number'] = config.get_field_value('tag_number')
        inputdict['interface_name'] = config.get_field_value('interface_name')
        inputdict['parameter'] = config.get_field_value('parameter')
        inputdict['entry_compare'] = config.get_field_value('entry_compare')
        inputdict['entry_counter_val'] = config.get_field_value('entry_counter_val')
        inputdict['poll_interval'] = config.get_field_value('poll_interval')
        # END OF FETCHING THE LEAF PARAMETERS

        # START OF FETCHING THE PARENT KEY LEAF PARAMETERS
        inputkeydict['managed_cpe_services_customer_eem_applets_event_manager_applet_applet_name'] = sdata.getRcPath().split('/')[-3].split('=')[1]
    ...
    def update(self, id, sdata):
        #Fetch Local Config Object
        config = getCurrentObjectConfig(id, sdata, 'tag', False)
        pconfig = getPreviousObjectConfig(id, sdata, 'tag')
        opaque_args = self.opaque_args
        ...
        pinputdict['tag_number'] = pconfig.get_field_value('tag_number')
        pinputdict['interface_name'] = pconfig.get_field_value('interface_name')
        pinputdict['parameter'] = pconfig.get_field_value('parameter')
        pinputdict['entry_compare'] = pconfig.get_field_value('entry_compare')
        pinputdict['entry_counter_val'] = pconfig.get_field_value('entry_counter_val')
        pinputdict['poll_interval'] = pconfig.get_field_value('poll_interval')
        dev = []
    ...
    @staticmethod
    def getInstance():
        if(Tag._instance == None):
            Tag._instance = Tag()
        return Tag._instance
