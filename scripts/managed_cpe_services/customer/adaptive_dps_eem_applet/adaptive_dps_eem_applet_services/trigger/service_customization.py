...
from servicemodel.controller.devices.device.eem_applets.event_manager_applet import trigger
...
def adaptive_dps_def(entity, conf, sdata, **kwargs):
    ...
    applet_events_obj = trigger.trigger()
    applet_events_obj.delay_time = inputdict['delay_time']
    applet_events_obj.occurrences_no = inputdict['occurrences_no']
    applet_events_obj.period = inputdict['period']
    applet_events_obj.period_start = inputdict['period_start']
    yang.Sdk.createData(device.url+"/eem-applets/event-manager-applet=%s" %(inputdict['applet_name']), applet_events_obj.getxml(filter=True), sdata.getSession())
