...
from servicemodel.controller.devices.device.eem_applets.event_manager_applet.trigger import correlate
...
def adaptive_dps_def(entity, conf, sdata, **kwargs):
    ...
    applet_events_obj.event2 = inputdict['event2']
    yang.Sdk.createData(device.url+"/eem-applets/event-manager-applet=%s/trigger" %(inputdict['applet_name']), applet_events_obj.getxml(filter=True), sdata.getSession())
