        ...
        if modify:
            ...
            if hasattr(obj.adaptive_dps_eem_applet_services, 'single_cpe_site'):
                if obj.adaptive_dps_eem_applet_services.single_cpe_site == "true":
                    print "Entered in to single site "
            ...
            adaptive_dps_events_tag_def(entity, conf, sdata, **kwargs)
...
def adaptive_dps_events_tag_def(entity, conf, sdata, **kwargs):
    interface_name = None
    mode = None
    device = None
    inputdict = kwargs['inputdict']
    ...
    applet_events_obj = eem_applets.event_manager_applet.events.tag.tag() 
    applet_events_obj.tag_number = inputdict['tag_number']
    applet_events_obj.interface_name = inputdict['interface_name']
    applet_events_obj.parameter = inputdict['parameter']
    applet_events_obj.entry_compare = inputdict['entry_compare']
    applet_events_obj.entry_counter_val = inputdict['entry_counter_val']
    applet_events_obj.poll_interval = inputdict['poll_interval']
    yang.Sdk.createData(device.url+"/eem-applets/event-manager-applet=%s/events" %(inputdict['applet_name']), applet_events_obj.getxml(filter=True), sdata.getSession())
