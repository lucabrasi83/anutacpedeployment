...
class ServiceDataCustomization:
    ...
    def process_service_create_data(smodelctx, sdata, dev, **kwargs):
        ...
        if modify:
            ...
            adaptive_dps_actions_action_def(entity, conf, sdata, **kwargs)
...
def adaptive_dps_actions_action_def(entity, conf, sdata,**kwargs):
    interface_name = None
    mode = None
    inputdict = kwargs['inputdict']
    if entity == 'cpe':
    ...
    #device = devicemgr.getDeviceById(inputdict['device_ip'])

    applet_action_obj = eem_applets.event_manager_applet.actions.action.action() 
    applet_action_obj.label = inputdict['label']
    applet_action_obj.action_statement = inputdict['action_statement']
    if inputdict['action_statement'] == "cli":
    	applet_action_obj.cli_type = inputdict['cli_type']
    	applet_action_obj.cli_string = inputdict['cli_string']
    if inputdict['action_statement'] == "comment":
    	applet_action_obj.comment_string = inputdict['comment_string']
    if inputdict['action_statement'] == "exit":
    	applet_action_obj.exit_result = inputdict['exit_result']
    if inputdict['action_statement'] == "handle-error":
    	applet_action_obj.handle_error_type = inputdict['handle_error_type']
    if inputdict['action_statement'] == "if":
    	applet_action_obj.first_operand = inputdict['first_operand']
    	applet_action_obj.second_operand = inputdict['second_operand']
    	applet_action_obj.compare = inputdict['compare']
    if inputdict['action_statement'] == "regexp":
    	applet_action_obj.regex_pattern = inputdict['regex_pattern']
    	applet_action_obj.input_string = inputdict['input_string']
    if inputdict['action_statement'] == "snmp-trap":
    ...
    yang.Sdk.createData(device.url+"/eem-applets/event-manager-applet=%s/actions" %(inputdict['applet_name']), applet_action_obj.getxml(filter=True), sdata.getSession())
