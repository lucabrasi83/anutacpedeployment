...
class SecondaryInetWan(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        config = getCurrentObjectConfig(id, sdata, 'secondary_inet_wan')
        ...
        if inputdict.get('name') is None:
          inputdict['name'] = 'secondary-inet-wan'
        inputdict['secondary_inet_wan_connectivity'] = config.get_field_value('secondary_inet_wan_connectivity')
        if inputdict.get('secondary_inet_wan_connectivity') is None:
          inputdict['secondary_inet_wan_connectivity'] = 'INET'
        # END OF FETCHING THE LEAF PARAMETERS

        # START OF FETCHING THE PARENT KEY LEAF PARAMETERS
        #inputkeydict['managed_cpe_services_customer_dual_cpe_dual_wan_site_dual_cpe_dual_wan_site_services_site_name'] = sdata.getRcPath().split('/')[-2].split('=')[1]
    ...
    @staticmethod
    def getInstance():
        if(SecondaryInetWan._instance == None):
            SecondaryInetWan._instance = SecondaryInetWan()
        return SecondaryInetWan._instance

    #def rollbackCreate(self, id, sdata):
        #        log('rollback: id = %s, sdata = %s' % (id, sdata))
        #        self.delete(id,sdata)

if __name__ == 'secondary_inet_wan':
  from servicemodel.yang import YangServiceData
  sdata = YangServiceData()
  instance = SecondaryInetWan().getInstance()
