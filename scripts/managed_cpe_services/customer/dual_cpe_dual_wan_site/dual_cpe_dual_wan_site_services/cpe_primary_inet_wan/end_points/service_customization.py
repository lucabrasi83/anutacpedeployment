...
class ServiceDataCustomization:
    ...
    def process_service_device_bindings(smodelctx, sdata, dev, **kwargs):
      ...
      if modify:
        ...
        for device in util.convert_to_list(dev):
            wan_endpoint('cpe_primary_inet_dual', smodelctx, sdata, device, **kwargs)

    @staticmethod
    def process_service_update_data(smodelctx, sdata, **kwargs):
      """callback called for update operation"""
      raise Exception('Update forbidden for node end-points at path managed-cpe-services/customer/dual-cpe-dual-wan-site/dual-cpe-dual-wan-site-services/cpe-primary-inet-wan/end-points')
      modify = True
      if modify and kwargs is not None:
        for key, value in kwargs.iteritems():
          log("%s == %s" %(key,value))
    ...
    def process_service_delete_data(smodelctx, sdata, **kwargs):
      ...
      if modify:
        ...
        dev = kwargs['dev']
        for device in util.convert_to_list(dev):
            delete_physical_interface('cpe_primary_inet_dual', smodelctx, sdata, device, **kwargs)
