...
class Switches(yang.AbstractYangServiceHandler):
    _instance = None
    ...
    def create(self, id, sdata):
        ...
        config = getCurrentObjectConfig(id, sdata, 'switches', False)
        ...
        if inputdict.get('sequence_number') is None:
          inputdict['sequence_number'] = '130'
        inputdict['device'] = config.get_field_value('device')
        # END OF FETCHING THE LEAF PARAMETERS

        #Fetch Device Object
        dev = getDeviceObject(config.get_field_value('device'), sdata)
    ...
    def update(self, id, sdata):
        #Fetch Local Config Object
        config = getCurrentObjectConfig(id, sdata, 'switches', False)
        pconfig = getPreviousObjectConfig(id, sdata, 'switches')
        opaque_args = self.opaque_args
        ...
        if pinputdict.get('sequence_number') is None:
          pinputdict['sequence_number'] = '130'
        pinputdict['device'] = pconfig.get_field_value('device')
        #Fetch Device Object
        dev = getDeviceObject(pconfig.get_field_value('device'), sdata)
        self.opaque_args['hireachy_device'] = dev
    ...
    @staticmethod
    def getInstance():
        if(Switches._instance == None):
            Switches._instance = Switches()
        return Switches._instance
